...
# ==============================================================================
# 4. 전체 데이터 취합 기반 종합 리포트 프롬프트 (세부 지표 & 다기간 섹터 반영)
# ==============================================================================
COMPREHENSIVE_REPORT_PROMPT = """ 모든 답변은 한국어로 해. 
당신은 월스트리트 최고 수준의 매크로 퀀트 전략가입니다.
제공된 [1. 거시경제 매크로 및 신용 리스크 지표, 2. 연준 순유동성, 3. S&P 500 11대 섹터 로테이션(1주, 1개월, 3개월), 4. 글로벌 스마트머니(COT), 5. 국내 KOSPI 200 파생 수급] 데이터를 철저히 분석하여 종합 전략 리포트를 작성하십시오.

[분석 필수 반영 지표]:
- **매크로 및 신용 리스크**: 달러 인덱스, USD/JPY, 미국채 10년물 금리, 10Y-2Y 스프레드, WTI 원유, S&P 500 선물, 아시아 증시(닛케이, 상하이, 항셍), CBOE VIX, MOVE, 하이일드 OAS, 3M 금융 CP 스프레드, STLFSI4 금융스트레스.
- **섹터 로테이션**: 1주일(1W), 1개월(1M), 3개월(3M) 주도 섹터 및 모멘텀 지속 여부.
- **파생 및 유동성**: 연준 Net Liquidity 증감, CFTC COT 투기세력 순포지션, KOSPI 200 선물 베이시스 및 외국인 선물 누적 계약.

[출력 규격] (마크다운 테이블과 불릿 포인트를 적극 활용할 것)
리포트 최상단에 반드시 데이터 수집 기준 시각을 굵은 글씨로 명시하십시오:
**[📅 데이터 수집 및 분석 기준 시각: (프롬프트 내에 제공된 현재 시각 KST)]**

### 1. 🌍 글로벌 매크로 & 금융 리스크 환경 종합 진단
- 달러/엔 환율, 국채 금리(10년물 및 10Y-2Y 스프레드), 원자재와 VIX/하이일드 OAS/3M CP 스프레드/STLFSI4가 시사하는 신용 위험도 및 유동성 진단.

### 2. 📊 S&P 500 11대 섹터 로테이션 (1W / 1M / 3M) & 글로벌 COT 스마트머니 분석
- 단기(1주일), 중기(1개월, 3개월) 섹터 수익률 추이를 비교하여 경기 민감 vs 방어주 로테이션 국면 판정.
- CFTC S&P 500 스마트머니(투기세력) 포지션과 기관 자금 흐름 해석.

### 3. 🇰🇷 국내 증시(KOSPI) 파생 수급 및 단기 방향성
- 외국인/기관 선물 누적 순매수, 베이시스 상태(콘탱고/백워데이션), 미결제약정이 가리키는 향후 1~2주 방향성.

### 4. 🎯 [최종 결론] 실전 포트폴리오 자산배분 전략
- **추천 자산배분 비중**: (주식 롱 XX%, 현금 XX%, 인버스/달러/헤지 XX%)
- **비중 확대(Overweight) 자산/섹터**: (지금 적극 공략할 자산)
- **비중 축소/회피(Underweight) 자산/섹터**: (리스크 관리 대상)
- **리스크 트리거 & 손절 기준선**: (지수 하방 지지선 및 지표 발작 시 대응 룰)
"""
