...
def render_liquidity_view():
    ...
    with tab3:
        ...
        with c1:
            st.markdown("##### 🏛️ :blue[1. 연준 총자산 (WALCL)]")
            st.markdown("• **증가 시:** 양적완화(QE)로 시중에 직접 달러 공급 ➔ :green[**유동성 증가 (+)**]")
            st.markdown("• **감소 시:** 양적긴축(QT)으로 만기 채권 미재투자 ➔ :red[**유동성 흡수 (-)**]")

        with c2:
            st.markdown("##### 🏦 :orange[2. 재무부 일반계좌 (TGA)]")
            st.markdown("• **잔고 증가:** 정부가 국채를 발행해 시장 돈을 흡수 ➔ :red[**유동성 감소 (-)**]")
            st.markdown("• **잔고 감소:** 정부가 재정 지출 및 보조금 집행 ➔ :green[**시장에 달러 방출 (+)**]")

        with c3:
            st.markdown("##### 🔄 :violet[3. 역레포 잔고 (ON RRP)]")
            st.markdown("• **잔고 증가:** MMF가 돈을 굴릴 곳이 없어 연준에 예치 ➔ :red[**유동성 잠김 (-)**]")
            st.markdown("• **잔고 감소:** 예치금을 빼서 국채 매수 및 시중 투자 ➔ :green[**유동성 방출 (+)**]")
