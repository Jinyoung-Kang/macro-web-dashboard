...
def render_liquidity_view():
    ...
    with tab3:
        ...
        st.markdown("""
        전통적으로 시장은 연준의 기준금리에 주목하지만, **중기 증시(S&P 500 / 나스닥)의 밸류에이션과 방향성을 결정하는 실질적인 힘은 '실제 시스템에 풀려 있는 달러 유동성의 크기'**입니다.
        ...
        with c1:
            st.markdown("##### 1. 연준 총자산 (WALCL)")
            st.write("• <span style='color: #07c94e;'>**증가 시:**</span> 양적완화(QE)로 시중에 직접 달러 공급 ➔ **유동성 증가 (+)**")
            st.write("• <span style='color: #07c94e;'>**감소 시:**</span> 양적긴축(QT)으로 만기 채권 미재투자 ➔ **유동성 흡수 (-)**")

        with c2:
            st.markdown("##### 2. 재무부 일반계좌 (TGA)")
            st.write("• **잔고 증가:** 정부가 국채를 발행해 시장 돈을 흡수 ➔ **유동성 감소 (-)**")
            st.write("• **잔고 감소:** 정부가 재정 지출 및 보조금 집행 ➔ **시장에 달러 방출 (+)**")

        with c3:
            st.markdown("##### 3. 역레포 잔고 (ON RRP)")
            st.write("• **잔고 증가:** MMF가 돈을 굴릴 곳이 없어 연준에 예치 ➔ **유동성 잠김 (-)**")
            st.write("• **잔고 감소:** 예치금을 빼서 국채 매수 및 시중 투자 ➔ **유동성 방출 (+)**")

        st.markdown("---")
        st.markdown("#### 🎯 실전 투자 체크포인트")
        st.write("1. **순유동성 증가 국면:** 주식 시장의 밸류에이션(PER) 확장 및 성장주/테크주 랠리 가능성 고조.")
        st.write("2. **순유동성 정체/감소 국면:** 지수 상단 제한, 변동성 확대 및 방어주/현금 비중 확대 전략 유리.")
        st.write("3. **TGA 재충전 구간 주의:** 부채한도 협상 타결 직후 재무부가 대규모 국채를 발행할 때 증시 단기 조정 압력 빈번.")
