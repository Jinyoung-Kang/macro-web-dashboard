# views/kiwoom_test_view.py
import streamlit as st
from services.kiwoom_service import (
    get_kiwoom_token,
    fetch_kiwoom_stock_quote,
    fetch_kiwoom_kospi_index
)
...
def render_kiwoom_test_view():
    ...
    with col_s1:
        shcode_input = st.text_input("조회할 종목코드 (6자리)", value="005930", key="kiwoom_shcode_input")
    with col_s2:
        st.write("")
    ...
    with st.spinner(f"종목코드 [{target_code}] 실시간 시세 수신 중..."):
        quote_data, quote_err = fetch_kiwoom_stock_quote(target_code)

    if quote_err:
        st.error(f"⚠️ **시세 조회 오류:** {quote_err}")
    elif quote_data:
        hname = quote_data.get("rprs_mrkt_kor_name", quote_data.get("hts_kor_isnm", "종목"))
        price = float(quote_data.get("stck_prpr", 0))
        diff = float(quote_data.get("prdy_vrss", 0))
        rate = float(quote_data.get("prdy_cttr", 0))
        volume = int(quote_data.get("acml_vol", 0))
        value = int(quote_data.get("acml_tr_pbmn", 0))
