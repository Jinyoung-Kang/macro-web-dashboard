...
def render_ai_test_view():
    ...
    st.caption("1순위 Cloudflare (DeepSeek-R1-32B), 2순위 Cerebras, 3순위 NVIDIA NIM의 우선순위 연결 및 한글 번역 파이프라인을 검증합니다.")
    st.divider()
    ...
    with st.expander("⚙️ Streamlit Secrets 인증 키 로드 상태 (클릭하여 확인)", expanded=True):
        ...
        with c2:
            st.markdown(f"**🥈 2순위 Cerebras:** {'🟢 로드 완료' if sec_ce_key else '🔴 미설정'}")
        ...
        with c3:
            ...
            nv_key = st.text_input("NVIDIA Key (번역/백업용)", value=sec_nv_key, type="password", key="ui_nv_key")
    ...
    if run_individual:
        ...
        with st.status("AI 엔진 개별 테스트 진행 중...", expanded=True) as status:
            st.write("1/3. 🥇 Cloudflare (DeepSeek-R1-32B + NVIDIA 한글 번역) 호출 중...")
            results["1순위: Cloudflare AI"] = test_cloudflare_ai(final_cf_id, final_cf_token, test_prompt, nv_key=final_nv)
            
            st.write("2/3. 🥈 Cerebras (GPT-OSS-120B) 호출 중...")
            results["2순위: Cerebras Cloud"] = test_cerebras(final_ce, test_prompt)
