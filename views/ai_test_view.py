...
def render_ai_test_view():
    ...
    # 엔진 연결 및 인증 키 로드 상태 4칸 배치 (순서 변경)
    ...
    if run_individual:
        ...
        with st.status("AI 엔진 개별 테스트 진행 중...", expanded=True) as status:
            ...
            results["1순위: NVIDIA Nemotron-3"] = test_nvidia_nemotron(sec_nv_key, test_prompt)
            
            st.write("2/4. 🥈 Cloudflare (DeepSeek-R1 + Llama-3.1 번역) 호출 중...")
            results["2순위: Cloudflare AI"] = test_cloudflare_ai(sec_cf_id, sec_cf_token, test_prompt)
